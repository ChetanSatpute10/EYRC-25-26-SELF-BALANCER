...
def sysCall_init():
    ...
    wheel_radius = 0.0215
    max_motor_velocity = 150.0  # rad/s
...
def sysCall_sensing():
    global SIM, robot_body, motor_left, motor_right
    global filtered_pitch_rate, filtered_wheel_velocity, previous_pitch
    global yaw_correction_factor, accumulated_pitch_error, integral_clamp_limit

    # orientation: [rotX, rotY, rotZ] (rotX = pitch)
    orientation = SIM.getObjectOrientation(robot_body, -1)
    raw_pitch = orientation[0] if orientation else 0.0

    # angular velocity
    _, angular_velocity = SIM.getObjectVelocity(robot_body)
    pitch_rate = angular_velocity[0] if angular_velocity else 0.0
    ...
    wheel_diff = left_wheel_speed - right_wheel_speed

    # Apply yaw correction
    corrected_pitch = raw_pitch - (yaw_correction_factor * (wheel_diff * 0.01))

    # Update filters
    filtered_pitch_rate = (filtered_pitch_rate * 0.975) + (pitch_rate * 0.025)
    filtered_wheel_velocity = (filtered_wheel_velocity * 0.975) + (avg_wheel_speed * 0.025)

    # Update integral pitch error
    pitch_error = -corrected_pitch
    ...
    previous_pitch = corrected_pitch
