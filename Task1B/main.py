...
def sysCall_actuation():
    ...
    # Get keyboard states from signals
    current_keys = {
        'up': 1 if SIM.getIntegerSignal('key_up') else 0,
        'down': 1 if SIM.getIntegerSignal('key_down') else 0,
        'left': 1 if SIM.getIntegerSignal('key_left') else 0,
        'right': 1 if SIM.getIntegerSignal('key_right') else 0
    }

    # Handle movement based on keys
    if current_keys['up']:
        velocity_linear_set_point = 0.01  # Forward movement
        yaw_cmd = 0.0
    elif current_keys['down']:
        velocity_linear_set_point = 0.0   # Stop
        yaw_cmd = 0.0
    elif current_keys['left']:
        velocity_linear_set_point = 0.0   # Turn in place
        yaw_cmd = 0.1
