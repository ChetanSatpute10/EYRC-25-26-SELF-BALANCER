...
def sysCall_init():
    ...
    # KEY DIFFERENCE: Initialize Kalman filters with tuned parameters

    # Code 2 uses exponential smoothing (alpha=0.05)

    # Code 1 uses Kalman filtering with noise covariance tuning

    # ============================================================

    

    # Kalman filter for pitch rate (angular velocity noise typically higher)

    self.kf_pitch_rate = KalmanFilter1D(

        process_variance=0.00001,      # System dynamics noise

        measurement_variance=0.001      # Sensor noise (gyroscope)

    )

    

    # Kalman filter for wheel velocity (encoder noise typically lower)

    self.kf_wheel_vel = KalmanFilter1D(

        process_variance=0.00001,      # System dynamics noise

        measurement_variance=0.0005     # Sensor noise (encoders are more accurate)

    )

    

    # Kalman filter for forward velocity (derived from IMU + encoders)
    ...
    # Filtered state variables (outputs of Kalman filters)
    ...
    self.forward_speed = 0.12       # Translation command scale

    self.turn_rate = 1.5            # Rotation command scale
    ...
    self.ki_coefficient = 0.0       # Set to non-zero to enable integral action

    

    print("Bot initialized - Using Kalman filtering for state estimation")

    print("Controls: Arrow keys for movement, W/S for arm, Space to stop")
...
def sysCall_sensing(): 
    ...
    Sensing function using Kalman filtering instead of exponential smoothing.

    This provides optimal filtering under Gaussian noise assumptions.
    ...
    # Compute pitch rate in local frame
    ...
    # KEY DIFFERENCE: Use Kalman filtering instead of exponential smoothing
    ...
    # Optional: Also filter forward velocity for smoother behavior

    self.forward_velocity_filtered = self.kf_forward_vel.update(self.forward_velocity_local)

    

    # Anti-windup accumulation (using filtered tilt angle)
    ...
    # Store last tilt for control

    self.last_tilt_angle = tilt_angle_body

    

    # Store raw wheel values for reference
...
def sysCall_actuation():
    ...
    Control function using Kalman-filtered state estimates.

    The filtering is done in sysCall_sensing(), this function uses the filtered values.
    ...
    # KEY DIFFERENCE: Use Kalman-filtered values (already computed in sensing)

    # Code 2 uses exponential smoothing; Code 1 uses Kalman filtering

    # ============================================================

    

    # Update position using Kalman-filtered forward velocity
    ...
    # Build LQR state vector using Kalman-filtered estimates

    x1 = 0.0 - self.last_tilt_angle           # Pitch error (tilt angle)

    x2 = -self.pitch_rate_filtered            # Pitch rate (Kalman filtered)

    x3 = position_error                        # Position error

    x4 = velocity_error                        # Velocity error

    

    # Compute LQR control law: u = -K*x
    ...
    # Add integral compensation (anti-windup)
